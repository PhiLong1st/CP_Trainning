...
struct info
{
    int first, second, pos, third, fourth;
};
...
void solve()
{
    ...
    cin >> n >> q;
    vector<pll> a(n + 1);
    vector<info> query(q + 1);
    Segment_Tree tree(1000);
    vector<int> ans(q + 1);
    for (int i = 1; i <= n; i++)
    {
    ...
    for (int i = 1; i <= q; i++)
    {
        cin >> query[i].first >> query[i].second >> query[i].third >> query[i].fourth;
        query[i].pos = i;
    }
    ...
    for (int i = 1; i <= q; i++)
    {
        while (vt <= n && a[vt].first < query[i].first)
        {
